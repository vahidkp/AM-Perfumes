    ...
    >
          ...
          {product.variants.map((v) => {
            ...
            return (
              <button
                ...
                aria-pressed={active}
                className={`rounded-full border px-6 py-2.5 font-body text-sm transition-all ${
                  active
                    ? 'border-gold bg-cream text-gold-deep'
                    : 'border-ink/20 bg-white text-ink hover:border-gold/50'
                }`}
              >
                {v.size}
              </button>
      ...
      <div className="mb-8 space-y-3">
        <div className="flex items-stretch gap-3">
          <div className="flex shrink-0 items-center overflow-hidden rounded-full border border-ink/20">
            <button
              onClick={() => setQuantity((q) => Math.max(1, q - 1))}
              className="flex h-14 w-11 items-center justify-center text-ink transition-colors hover:bg-ink/5"
              aria-label="Decrease quantity"
            >
              −
            </button>
            <span className="flex h-14 w-9 items-center justify-center font-body text-ink">
              {quantity}
            </span>
            <button
          ...
          <button
            ...
            disabled={adding}
            className="flex h-14 flex-1 items-center justify-center gap-2 rounded-full border border-gold/50 bg-emerald px-4 font-body text-sm font-semibold uppercase tracking-wider text-cream shadow-[0_12px_34px_-16px_rgba(12,58,46,0.8)] transition-all hover:border-gold hover:bg-emerald-light disabled:opacity-70"
          >
        ...
        <button
          ...
          className="flex h-14 w-full items-center justify-center gap-2 rounded-full border border-emerald/30 font-body text-sm uppercase tracking-wider text-emerald transition-all hover:bg-emerald hover:text-ivory"
        >
          Buy Now
        </button>
      </div>
      ...
      {/* Secure payment */}
      <div className="mb-8 flex flex-col items-center gap-3 border-t border-ink/10 pt-6 sm:flex-row sm:justify-between">
        <span className="font-body text-[11px] uppercase tracking-[0.15em] text-ink-soft">
